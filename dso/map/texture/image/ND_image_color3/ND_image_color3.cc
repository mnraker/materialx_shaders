...
/// @file ND_image_color3.cc

#include "attributes.cc"
#include "ND_image_color3_ispc_stubs.h"
...
RDL2_DSO_CLASS_BEGIN(ND_image_color3, Map)
public:
    ND_image_color3(SceneClass const &sceneClass, std::string const &name);
    void update();

private:
    static void sample(const Map *self, moonray::shading::TLState *tls,
                       const moonray::shading::State &state, Color *sample);
    ispc::ND_image_color3 mIspc;
    
    std::unique_ptr<moonray::shading::BasicTexture> mTexture;

RDL2_DSO_CLASS_END(ND_image_color3)

ND_image_color3::ND_image_color3(const SceneClass& sceneClass,
        const std::string& name) :
    Parent(sceneClass, name)
{
    mSampleFunc = ND_image_color3::sample;
    mSampleFuncv = (SampleFuncv) ispc::ND_image_color3_getSampleFunc();

    mTexture = std::make_unique<moonray::shading::BasicTexture>(this, sLogEventRegistry);
}

void
ND_image_color3::update()
{
    ...
    mIspc.mDefaultColor = asIspc(get(defaultAttr));
    ...
    if (hasChanged(fileAttr) ||
        ...
        if (!mTexture->update(get(fileAttr),
                              static_cast<ispc::TEXTURE_GammaMode>(2), // auto gamma
                              wrapS,
                              wrapT,
...
void
ND_image_color3::sample(const Map* self, moonray::shading::TLState *tls,
                 const moonray::shading::State& state, Color* sample)
{
    const ND_image_color3* me = static_cast<const ND_image_color3*>(self);
