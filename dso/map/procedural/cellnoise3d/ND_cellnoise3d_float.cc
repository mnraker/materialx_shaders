...
/// @file ND_cellnoise3d_float.cc

#include "attributes.cc"
#include "ND_cellnoise3d_float_ispc_stubs.h"
...
RDL2_DSO_CLASS_BEGIN(ND_cellnoise3d_float, Map)
public:
    ND_cellnoise3d_float(SceneClass const &sceneClass, std::string const &name);
    void update();

private:
    static void sample(const Map *self, moonray::shading::TLState *tls,
                       const moonray::shading::State &state, Color *sample);
    ispc::ND_cellnoise3d_float mIspc;
    std::unique_ptr<moonray::noise::Noise> mNoise;

RDL2_DSO_CLASS_END(ND_cellnoise3d_float)

ND_cellnoise3d_float::ND_cellnoise3d_float(const SceneClass& sceneClass,
        const std::string& name) :
    Parent(sceneClass, name)
{
    mSampleFunc = ND_cellnoise3d_float::sample;
    mSampleFuncv = (SampleFuncv) ispc::ND_cellnoise3d_float_getSampleFunc();
}

void
ND_cellnoise3d_float::update()
{
...
void
ND_cellnoise3d_float::sample(const Map* self, moonray::shading::TLState *tls,
                 const moonray::shading::State& state, Color* sample)
{
    const ND_cellnoise3d_float* me = static_cast<const ND_cellnoise3d_float*>(self);
