...
/// @file ND_unifiednoise3d_float.cc

#include "attributes.cc"
#include "ND_unifiednoise3d_float_ispc_stubs.h"
...
RDL2_DSO_CLASS_BEGIN(ND_unifiednoise3d_float, Map)
public:
    ND_unifiednoise3d_float(SceneClass const &sceneClass, std::string const &name);
    void update();

private:
    static void sample(const Map *self, moonray::shading::TLState *tls,
                       const moonray::shading::State &state, Color *sample);
    ispc::ND_unifiednoise3d_float mIspc;
...
RDL2_DSO_CLASS_END(ND_unifiednoise3d_float)

ND_unifiednoise3d_float::ND_unifiednoise3d_float(const SceneClass& sceneClass,
        const std::string& name) :
    Parent(sceneClass, name)
{
    mSampleFunc = ND_unifiednoise3d_float::sample;
    mSampleFuncv = (SampleFuncv) ispc::ND_unifiednoise3d_float_getSampleFunc();
}

void
ND_unifiednoise3d_float::update()
{
...
void
ND_unifiednoise3d_float::sample(const Map* self, moonray::shading::TLState *tls,
                 const moonray::shading::State& state, Color* sample)
{
    const ND_unifiednoise3d_float* me = static_cast<const ND_unifiednoise3d_float*>(self);

    const Vec3f position = evalVec3f(me, positionAttr, tls, state);
    const Vec3f freq = evalVec3f(me, freqAttr, tls, state);
    const Vec3f offset = evalVec3f(me, offsetAttr, tls, state);
