...
MTLX_ATTR_DECLARE_BEGIN
    ...
    AttributeKey<IN_TYPE> valuebAttr;
    AttributeKey<IN_TYPE> valuetAttr;
    AttributeKey<Float>   centerAttr;
...
MTLX_ATTR_DEFINE_BEGIN
    ...
    valuebAttr   = sceneClass.declareAttribute<IN_TYPE>("valueb",   IN_DEFAULT,  FLAGS_BINDABLE, INTERFACE_GENERIC, {});
    valuetAttr   = sceneClass.declareAttribute<IN_TYPE>("valuet",   IN_DEFAULT,  FLAGS_BINDABLE, INTERFACE_GENERIC, {});
    centerAttr   = sceneClass.declareAttribute<Float>  ("center",   0.5f,        FLAGS_BINDABLE, INTERFACE_GENERIC, {});
                 ...
                 const moonray::shading::State& state, RETURN_TYPE* sample)
{
    ...
    const IN_TYPE valueb = IN_EVAL  (me, valuebAttr, tls, state);
    const IN_TYPE valuet = IN_EVAL  (me, valuetAttr, tls, state);
    const float   center = evalFloat(me, centerAttr, tls, state);
    ...
    if (me->mIspc.disableMode) {
        ...
        // But the same applies to the 'center' attribute, so the assignment should be
        //     out = (1-v < 1-center) ? valueb : valuet;
        // However, this can be simplified to the following:
        out = (v < center) ? valuet : valueb;
    }
