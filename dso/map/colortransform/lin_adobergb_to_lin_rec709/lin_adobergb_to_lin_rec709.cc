...
/// @file lin_adobergb_to_lin_rec709.cc
...
#define OPERATION lin_adobergb_to_lin_rec709
...
IN_TYPE
doLinAdobergbToLinRec709(IN_TYPE inValue)
{
    // Adobe RGB (1998) and Rec.709/sRGB share the same primaries (both D65 white point)
    // When both are already linear, they are identical - no transformation needed
    return inValue;
}
                    ...
                    const moonray::shading::State& state, RETURN_TYPE* sample)
{
    ...
    if (!me->mIspc.disableMode) {
        outValue = doLinAdobergbToLinRec709(inValue);
    }
